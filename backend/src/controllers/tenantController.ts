...
const updateTenant = async (req: Request, res: Response) => {
  const { name, defaultLocale, theme, domains, allowedDomains } = req.body
  const t = await tenantService.update(req.params.id as string, { name, defaultLocale, theme, domains, allowedDomains })
  if (!t) return res.status(404).json({ error: 'not found' })
  res.json(t)
}
