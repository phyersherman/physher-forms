...
exports.copyFromTemplate = exports.assignCourseToTenant = void 0;
const client_1 = __importDefault(require("../db/client"));
/**
 * Helper function to copy course structure (chapters, modules, blocks)
 * Returns ID mappings for prerequisite remapping
 */
const copyCourseStructure = async (sourceCourse, newCourse, tenantId) => {
 ...
 */
const remapPrerequisites = async (sourceCourse, idMappings, logPrefix) => {
    ...
    for (const chapter of sourceCourse.chapters) {
        ...
        if (chapter.prerequisite_chapter_ids?.length) {
            ...
            if (remappedChapterPrereqs.length > 0) {
                console.log(`[${logPrefix}] Updating chapter ${newChapterId} prerequisites: ${remappedChapterPrereqs}`);
        ...
        for (const module of chapter.modules) {
            ...
            if (newModuleId && module.prerequisite_module_ids?.length) {
                ...
                if (remappedModulePrereqs.length > 0) {
                    console.log(`[${logPrefix}] Updating module ${newModuleId} prerequisites: ${remappedModulePrereqs}`);
...
const assignCourseToTenant = async (globalCourseId, tenantId, overrideTitle) => {
    try {
        ...
        console.log(`[assignCourseToTenant] Created ${idMappings.chapterIdMap.size} chapters and ${idMappings.moduleIdMap.size} modules`);
        // Remap prerequisites
...
exports.assignCourseToTenant = assignCourseToTenant;
/**
...
const copyFromTemplate = async (templateId, data) => {
    try {
        ...
        console.log(`[copyFromTemplate] Created ${idMappings.chapterIdMap.size} chapters and ${idMappings.moduleIdMap.size} modules`);
        // Remap prerequisites
