 ...
 */
async function canRequestMagicCode(userId) {
 ...
 */
async function generateMagicCode(userId, tenantId) {
    // Check rate limiting
    ...
    try {
        await (0, emailService_1.sendMagicCodeEmail)(user.email, user.fullName || 'User', code, organizationName, userTenantId);
 ...
 */
async function generateMagicCodeByEmail(email, tenantId) {
 ...
 */
async function validateMagicCode(email, code, tenantId) {
    ...
    if (!magicCode) {
        ...
        console.log(`[Magic Code Validation] Looking for codes with userId=${user.id}, got codes:`, await prisma.magicCode.findMany({
 ...
 */
async function getActiveCodesForUser(userId) {
