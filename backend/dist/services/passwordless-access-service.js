 ...
 */
async function createPasswordlessLink(data) {
 ...
 */
async function getPasswordlessLinks(tenantId, courseId) {
    ...
    const linksWithCreators = await Promise.all(links.map(async (link) => {
        const creator = await prisma.user.findUnique({
            where: { id: link.createdBy },
            select: { id: true, fullName: true, email: true },
        });
 ...
 */
async function getPasswordlessLinkById(linkId) {
 ...
 */
async function validatePasswordlessToken(token) {
    const link = await prisma.passwordlessAccessLink.findUnique({
        where: { token },
    });
    if (!link) {
 ...
 */
async function registerViaPasswordlessLink(data, ipAddress) {
    ...
    const enrollmentPromises = link.courseIds.map((courseId) => prisma.enrollment.create({
        data: {
            userId: user.id,
            tenantId: link.tenantId,
            courseId,
 ...
 */
async function updatePasswordlessLink(linkId, data) {
    const link = await prisma.passwordlessAccessLink.findUnique({
        where: { id: linkId },
    });
    if (!link) {
 ...
 */
async function togglePasswordlessLink(linkId) {
    const link = await prisma.passwordlessAccessLink.findUnique({
        where: { id: linkId },
    });
    if (!link) {
 ...
 */
async function deletePasswordlessLink(linkId) {
    const link = await prisma.passwordlessAccessLink.findUnique({
        where: { id: linkId },
    });
    if (!link) {
