...
const GlobalEmailConfigPage: React.FC = () => {
  const { user } = useAuth()
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (config) {
        ...
        alert('Global email configuration updated successfully')
      } else {
        // Create new (no tenantId = global)
        await api.createEmailConfig(data)
        alert('Global email configuration created successfully')
      }
  ...
  const handleDelete = async () => {
    ...
    if (!confirm('Are you sure you want to delete the global email configuration?')) {
      return
    }

    try {
      await api.deleteEmailConfig(config.id)
      alert('Global email configuration deleted successfully')
      setConfig(null)
      setEditing(true)
    } catch (err: any) {
  ...
  return (
    <AdminLayout title="Global Email Configuration">
      <div className={styles.header}>
        <div>
          <p className={styles.subtitle}>
            Configure default email settings for all tenants
          </p>
        </div>
        {config && !editing && (
      ...
      {loading ? (
        ...
        <div style={{ maxWidth: '800px' }}>
          ...
          >
            <strong style={{ color: '#1565c0' }}>ℹ️ Global Configuration</strong>
            <p style={{ margin: '8px 0 0 0', fontSize: '14px', color: '#1976d2' }}>
              This is the default email configuration used by all tenants that don't have their own configuration.
              Tenant-specific configurations override this global setting.
            </p>
          </div>
          ...
          >
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <small style={{ color: '#666', fontSize: '12px', marginLeft: '24px', display: 'block' }}>
                Enable or disable global email sending
              </small>
            </div>
...
export default GlobalEmailConfigPage
